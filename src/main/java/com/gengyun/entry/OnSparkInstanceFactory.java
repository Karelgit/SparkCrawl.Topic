...
public class OnSparkInstanceFactory {
    ...
    public OnSparkInstanceFactory(InitSparkConfig obj) {
        SPARK_CONFIG = obj;
        sparkConf = SPARK_CONFIG.getSparkConf();
        sparkContext = new JavaSparkContext(sparkConf);
        sc = new SQLContext(sparkContext);
    ...
    public static OnSparkInstanceFactory getInstance(final InitSparkConfig object) {
        if (singleton == null) {
            synchronized (OnSparkInstanceFactory.class) {
                if (singleton == null) {
                    singleton = new OnSparkInstanceFactory(object);
                }
            }
        }
        return singleton;
    ...
    private static ProtocolFilter PROTOCOL_FILTER = new ProtocolFilter();
    ...
    public static RDDCrawledQueue getRddCrawledQueue() {
        return RDD_CRAWLED_QUEUE;
    }
